...
import { listPendingApprovals, resolveApproval } from '@/lib/actions/approvals';
...
export default async function ApprovalsPage({
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl space-y-6">
      <AppPageHeader
        title="Approvals"
        description="High-blast-radius actions waiting on human go/no-go: file_sar, freeze_account. Decisions are recorded immutably."
      />
      ...
      {pending.length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              When an analyst or agent requests an HBR action (file_sar,
              freeze_account), it lands here for human approval.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : (
                  ...
                  <CardDescription>
                    ...
                    {p.transaction_id && (
                      <>
                        {' · '}
                        <Link
                          href={`/transactions/${p.transaction_id}`}
                          className="text-primary hover:underline"
                        >
                          source tx
                        </Link>
              ...
              <form className="grid sm:grid-cols-[1fr_auto_auto] gap-3 items-end">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    id={`note-${p.id}`}
                    name="note"
                    placeholder="Verified with senior analyst · checked LATAM list"
                  />
                </div>
