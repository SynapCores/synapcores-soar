...
export default async function McpSessionsPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl space-y-6">
      <AppPageHeader
        title="MCP auditor sessions"
        description="Every external auditor / examiner that's been minted a token, plus their per-token query count + last activity. Click a token to see every individual tool call."
      />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Hand the auditor this URL + their token (from{' '}
        ...
        <CardContent>
          <code className="block break-all rounded-md bg-black/40 px-3 py-2 text-sm">
            {process.env.NEXTAUTH_URL ?? 'http://localhost:3001'}/api/v1/mcp
          </code>
          <p className="text-xs text-muted-foreground mt-2">
            JSON-RPC 2.0 over POST. Auth: <code>Authorization: Bearer mcp_...</code>.
            Speaks <code>initialize</code>, <code>tools/list</code>, <code>tools/call</code>.
          </p>
        </CardContent>
      </Card>
