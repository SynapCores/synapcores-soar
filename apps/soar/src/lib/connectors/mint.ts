/**
 * Mint a connector token. Stored hashed in soar_integrations under
 * provider='connector_<source>'. The plaintext is shown to the user
 * exactly once (mirror of /settings/api-keys + /settings/mcp-tokens).
 */
...
export type ConnectorProvider = 'splunk' | 'sentinel' | 'crowdstrike' | 'okta';
...
export async function revokeConnector(id: string): Promise<void> {
  const db = getAdminClient();
  await db.sql(
    `UPDATE soar_integrations SET enabled = FALSE, updated_at = NOW() WHERE id = $1`,
    [id],
  );
}
